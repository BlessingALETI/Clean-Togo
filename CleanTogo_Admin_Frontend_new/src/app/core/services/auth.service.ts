...
export class AuthService {
  ...
  getCurrentAdmin() {
  return this.store.getAdminAccount();
}

updateCurrentAdmin(fullName: string, email: string, role: string) {
  this.store.updateAdminAccount({ fullName, email, role });

  // optionnel: si tu affiches l’email en haut dans la barre, tu peux le relire depuis store
}

changePassword(oldPass: string, newPass: string) {
  return this.store.changeAdminPassword(oldPass, newPass);
}
}
