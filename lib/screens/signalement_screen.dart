...
class _SignalementScreenState extends State<SignalementScreen> {
  ...
  @override
  void initState() { super.initState(); _loadReclamations(); }
  @override
  void dispose() { _objetCtrl.dispose(); _descCtrl.dispose(); super.dispose(); }

  Future<void> _loadReclamations() async {
    setState(() => _loadingList = true);
    try {
      final data = await _api.getAllReclamations();
      setState(() { _reclamations = data; _loadingList = false; });
    } catch (_) {
  ...
  Future<void> _envoyer() async {
    ...
    try {
      final rec = ReclamationModel(
        objet: _selectedObjet ?? _objetCtrl.text.trim(),
        description: _descCtrl.text.trim(),
      );
      await _api.saveReclamation(rec);
      if (!mounted) return;
      setState(() { _sending = false; _selectedObjet = null; _objetCtrl.clear(); _descCtrl.clear(); });
            ...
            Container(
              ...
              child: Form(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  // Objet (dropdown)
                  const Text('Objet', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: AppColors.textGrey)),
                  ...
                  Container(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        isExpanded: true,
                        hint: const Text('Sélectionnez un objet', style: TextStyle(color: AppColors.grey)),
                        value: _selectedObjet,
                        items: _objets.map((o) => DropdownMenuItem(value: o, child: Text(o, style: const TextStyle(fontSize: 14)))).toList(),
                        onChanged: (v) => setState(() { _selectedObjet = v; _objetCtrl.text = v ?? ''; }),
                      ),
                    ),
                  ),
                  const SizedBox(height: 16),
                  ...
                  // Description
                  const Text('Description', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: AppColors.textGrey)),
                  const SizedBox(height: 8),
                  TextFormField(
            ...
            _loadingList
                ...
                : _reclamations.isEmpty
                    ? Container(
                        ...
                        child: const Center(child: Text('Aucun signalement', style: TextStyle(color: AppColors.textGrey))))
                    : Column(children: _reclamations.map((r) => Container(
                        ...
                        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          ...
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Text(r.objet, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: AppColors.textDark)),
                            const SizedBox(height: 4),
                            Text(r.description, style: const TextStyle(color: AppColors.textGrey, fontSize: 12), maxLines: 2, overflow: TextOverflow.ellipsis),
                          ])),
                        ]),
                      )).toList()),
